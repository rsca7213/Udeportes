...
router.route('/:id_deporte/:id_categoria/:id/participaciones')
  /*
    Ruta GET que devolvera todas las participaciones de un entrenamiento con los siguientes datos:
  ...
  */
  .get(mw_token, async (req, res) => {
    try {
      ...
      // Si tiene acceso a la categoria
      if (check_permiso) {
        let data = await entrenamientos.obtenerParticipaciones(req.params.id_deporte, req.params.id_categoria, req.params.id);
  ...
  /*
    Ruta POST que guardara el registro de asistencia de un entrenamiento, recibiendo la siguiente data en el body:
    [ { cedula, asistencia = NULL || Boolean } ]
    siempre y cuando el usuario tenga acceso al contenido y el entrenamiento exista.
    Ruta protegida por MW_TOKEN
  */
  .post(mw_token, async (req, res) => {
    try {
      ...
      // Si tiene acceso a la categoria
      if (check_permiso) {
        let data = await entrenamientos.guardarParticipaciones(req.params.id_deporte, req.params.id_categoria, req.params.id, req.body.data);
        res.status(data.codigo).send(data.texto);
      }
      // Si no tiene acceso a la categoria
      else res.status(423).send('No posee una asignación con esta categoria.');
