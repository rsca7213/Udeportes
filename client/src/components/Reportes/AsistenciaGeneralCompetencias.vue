  ...
  <div>
    <v-row class="justify-center">
      <v-col class="pt-0 pl-md-11" cols="12" sm="10" lg="6" xl="6">
        <v-select v-model="periodo" label="Periodo" prepend-icon="mdi-calendar" :items="periodos"
        clear-icon="mdi-close" name="periodo" clearable hide-details>
        </v-select>
      </v-col>
    </v-row>
    <v-row v-if="!periodo">
      <v-col class="grey--text text-center"> Selecciona un período.
      </v-col>
    </v-row>
    <v-row class="justify-center" v-if="periodo && periodo ==='m'">
      <v-col class="pt-0 pl-md-11" cols="12" sm="10" lg="6" xl="6">
        <v-menu ref="menu" v-model="menu" :close-on-content-click="false" 
          transition="scale-transition" offset-y max-width="290px" min-width="auto">
          <template v-slot:activator="{ on, attrs }">
            <v-text-field v-model="formato_fecha" label="Mes" prepend-icon="mdi-calendar" readonly   v-bind="attrs" v-on="on" hide-details></v-text-field>
          </template>
          <v-date-picker ref="picker" color="primary" v-model="fecha" type="month" no-title scrollable @change="guardar" >
          </v-date-picker>
        </v-menu>
      </v-col>
    </v-row>
    <v-row v-if="periodo === 'm' && fecha === ''">
        ...
        <v-col cols="12" lg="9" xl="8" class="elevation-4 py-4 px-0 px-sm-6 rounded-lg">
          ...
          <v-row>
            <v-col cols="12"> 
              ...
              no-data-text="No hay atletas que hayan asistido a este competencia."
    ...
    <v-row v-else-if="(periodo === 't' || (periodo ==='m' && formato_fecha !=='')) && competencias===false &&!tabla_cargando && mensaje_error===''">
...
export default {
  name: 'AsistenciaGeneralCompetencias',
  components: {
    ApexChart
  },
  props:{
  ...
  watch: {
    ...
    categoria(){
      this.atletas=[];
      this.periodo = 0;
      this.competencias = false;
      this.mensaje_error='';
    },
    fecha(){
      this.atletas=[];
      this.formato_fecha = this.formatoFecha(this.fecha)
    },
    formato_fecha(){
      let param_fecha = []
      if(this.fecha!==''){
  ...
  methods: {
    formatoFecha (fecha) {
        ...
        return `${meses.filter(m => m.valor === mes)[0].nombre} (${year})`
    },
    ...
    getReporte(){
      var docDefinition = {
        content: [
          ...
          {text:'Asistencia General a Competencias', bold:true, color: '#2196F3', fontSize: 16, margin: [ 0, 10, 0, 10 ]},
          {text: [{text:'Equipo: ', bold:true, color: '#2196F3'},{text:`${this.equipo}`}], fontSize: 14, margin: [ 0, 10, 0, 0 ]},
          {text: [{text:'Período: ', bold:true, color: '#2196F3'},{text:`${this.periodo==='t'? 'Total': this.formato_fecha}`}], fontSize: 14, margin: [ 0, 5, 0, 20 ]},
    ...
    datosReporte(){
      ...
      reporte_body.push([
         ...
         { text: 'Asistencias', bold: true, color:'#2196F3', alignment:'center' },
         { text: 'Inasistencias', bold: true, color:'#2196F3', alignment:'center' },
         { text: '% Asistencia', bold: true, color:'#2196F3', alignment:'right'},
      ]);
      ...
      this.atletas.forEach((atleta) =>{
        reporte_body.push([
          ...
          {text: `${atleta.asistencias}`,  alignment:'center'},
          {text: `${atleta.faltas}`,  alignment:'center'},
          {text:`${atleta.porcentaje}`, alignment:'right'}
        ])
      });
    ...
    calcularRatios() {
      this.show = false;
      // Generamos un arreglo con la cantidad de atletas que asistieron y faltaron (sumados) por competencia
      let total = this.atletas.map(item => 
        parseInt(item.asistencias.replace(' Asistencia', '').replace(' Asistencias', '')) +
        parseInt(item.faltas.replace(' Falta', '').replace(' Faltas', '')) 
      );
      ...
      // Generamos un arreglo con la cantidad de atletas que asistieron por competencia
      let totalAsistencias = this.atletas.map(item => parseInt(item.asistencias.replace(' Asistencia', '').replace(' Asistencias', '')));
    ...
    async getAtletas(periodo, fecha=null) {
      ...
      this.mensaje_error= '';
      await axios.post(`${server_url}/reportes/asistencia/competencias/${this.categoria.id_deporte}/${this.categoria.id_categoria}`,{periodo, fecha}, { withCredentials: true } )
        .then((res) => {
          // En caso de exito
          if (res.status === 200) {
