<template>
  <div>
    ...
    <v-row class="justify-center mt-3 mx-2" v-if="mensaje_error">
    ...
    <div v-if="competencia && atletas.length && mensaje_error===''">
      <v-row no-gutters>
    ...
    <v-row v-else-if="!atletas.length && competencia  && !tabla_cargando && mensaje_error===''">
...
export default {
  name: 'AsistenciaDetalladacompetencias',
  components: {
    ApexChart
  },
  props:{
  ...
  methods: {
    ...
    getReporte(){
      var docDefinition = {
        content: [
          ...
          {text:'Asistencia a Competencia en Detalle', bold:true, color: '#2196F3', fontSize: 16, margin: [ 0, 10, 0, 10 ]},
    ...
    calcularRatios() {
      ...
      let atletas_asistentes = this.atletas.filter(atleta => atleta.asistencia === 'Sí').length;
      let atletas_inasistentes = this.atletas.filter(atleta => atleta.asistencia === 'No').length;

      let total = atletas_asistentes + atletas_inasistentes;


      // Calculamos los ratios
      this.ratioAsistencias = ((atletas_asistentes/total)*100|| 0).toFixed(2);
      this.ratioInasistencias = (((atletas_inasistentes)/total)*100|| 0).toFixed(2);
      this.show = true;
    },
    ...
    async getAtletas() {
      ...
      this.mensaje_error = '';
      await axios.get(`${server_url}/reportes/asistencia/competencia/${this.categoria.id_deporte}/${this.categoria.id_categoria}/${this.competencia.id_competencia}`, { withCredentials: true } )
        .then((res) => {
          // En caso de exito
          if (res.status === 200) {
