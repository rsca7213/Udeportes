<template>
  <v-dialog v-model="dialog" class="text-center" max-width="600">
    <template v-slot:activator="{ on, attrs }">
      <v-icon dense color="primary" dark v-bind="attrs" v-on="on"> mdi-clipboard-list </v-icon>
    </template>
    <v-card rounded="md">
      <v-card-title>
...
export default {
  ...
  methods: {
    ...
    */
    async sendData () {
      ...
      // request POST
      await axios.post(`${server_url}/entrenamientos/${this.id_deporte}/${this.id_categoria}/${this.id}/participaciones`, 
    ...
    */
    clearDialog(success = false) {
      ...
      this.mensajeError = '';
      if (success) {
        this.$emit('asistenciasGuardadas');
      }
      else {
  ...
  async mounted() {
    // Colocamos el loader
      ...
      this.cargando = true;
      // Request GET
      await axios.get(`${server_url}/entrenamientos/${this.id_deporte}/${this.id_categoria}/${this.id}/participaciones`,
