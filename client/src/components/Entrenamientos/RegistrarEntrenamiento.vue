...
<span>
  ...
  <v-snackbar v-model="snackbar" timeout="3000" shaped top>
    ...
    <span class="success--text"> ¡El entrenamiento fue registrado con éxito! </span>
...
export default {
  ...
  methods: {
    ...
    /*
      Funcion que hace la solicitud POST al servidor para registrar un entrenamiento,
    ...
    */
    async sendData () {
      ...
      // request POST
      await axios.post(`${server_url}/entrenamientos/${this.id_deporte}/${this.id_categoria}`, this.inputs, { withCredentials: true })
        .then((res) => {
          // Exito 200
    ...
    */
    clearDialog(success = false) {
      ...
      if (success) {
        this.snackbar = true;
        this.$emit('entrenamientoRegistrado');
      }
    },
